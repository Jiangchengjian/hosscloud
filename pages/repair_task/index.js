...
Page({
  ...
  onLoad: function (options) {
    ...
    this.getRepairLevel();
    this.setData({
      chooseText: wx.getStorageSync('chooseText')
    })
    },
   ...
   * 获取楼宇
   */
  getLocationProject: function () {
    
    const _this = this;
    if (_this.data.chooseText) {
      let data = {}
      ias.requestLoading('POST', ias.api.serviceLocation, JSON.stringify(data).encode(), '', function (res) {
        wx.setStorageSync('locationProject', res.results)
        wx.navigateTo({
          url: './location_project/index'
        })
      }, function () {
        wx.showToast({
          title: '加载项目失败',
